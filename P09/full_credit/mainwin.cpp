...
Mainwin::Mainwin() {
	 ...
	 //New Teacher
	 Gtk::MenuItem *menuItem_teacher = Gtk::manage(new Gtk::MenuItem("_New Teacher", true));
	 ...
	 menuItem_view_teachers->signal_activate().connect([this] {this->on_view_teacher_click();});
	 ...
	 //Insert Teachers
	 Gtk::Image *add_t_image = Gtk::manage(new Gtk::Image{""});
	 Gtk::ToolButton *insert_teacher_button = Gtk::manage(new Gtk::ToolButton(*add_t_image));
	 insert_teacher_button->set_tooltip_markup("Add new teacher.");
	 insert_teacher_button->signal_clicked().connect([this] {this->on_new_teacher_click();});
	 tools->append(*insert_teacher_button);
...
void Mainwin::on_new_section_click() { 
	...
	if((_courses.size() >= 1) && (_teachers.size() >= 1)) {
		Gtk::Dialog dialog{"Create Section", *this};
		Gtk::Grid plot;
		
		Gtk::Label l_course{"Select Course"};
		ComboBoxContainer<std::vector<Course*>> c_course(_courses);
		...
		Gtk::Label l_teacher{"Select Teacher"};
		ComboBoxContainer<std::vector<Teacher*>> c_teacher(_teachers);
		
		plot.attach(l_teacher, 0, 3, 1, 1);
		plot.attach(c_teacher, 1, 3, 2, 1);
		...
		int response;
		if((response = dialog.run()) == Gtk::RESPONSE_OK) {
			_sections.push_back( new Section{ *_courses[c_course.get_active_row_number()], semester_list(c_semester.get_active_row_number()), s_b_year.get_value_as_int()/*, _teachers[c_teacher.get_active_row_number()]*/ } );
		}
	} else { 
		if(_courses.size() >= 1){
			...
			Gtk::MessageDialog error{ *this, "No Teachers Available"};
			error.run();
		}
	}
...
void Mainwin::on_new_teacher_click() {
	...
	Gtk::Dialog dialog{"Teacher Info", *this};
	Gtk::Grid plot;
	...
	int response;
	if((response = dialog.run()) == Gtk::RESPONSE_OK) {
		_teachers.push_back( new Teacher{ e_name.get_text(), e_email.get_text() } );
	}
...
//	Teacher t = Teacher{ist};
	
	
//}

void Mainwin::on_student_parent_click() {
	Gtk::Dialog dialog{"Student Parent Relate", *this};
	Gtk::Grid plot;
	
	Gtk::Label l_student{"Select Student"};
	ComboBoxContainer<std::vector<Student*>> s_t(students);
	
	plot.attach(l_student, 0, 0, 1, 1);
	plot.attach(s_t, 1, 0, 2, 1);
	
	Gtk::Label l_parent{"Select Parent"};
	ComboBoxContainer<std::vector<Parent*>> p_t(parents);
	
	plot.attach(l_parent, 0, 1, 1, 1);
	plot.attach(p_t, 1, 1, 2, 1);
	...
	int response;
	if((response = dialog.run()) == Gtk::RESPONSE_OK) {
		students[s_t.get_active_row_number()]->add_parent(*parents[p_t.get_active_row_number()]);
		parents[p_t.get_active_row_number()]->add_student(*students[s_t.get_active_row_number()]);
	}
...
void Mainwin::on_save_click() {
	...
	try{
		...
		ofs << std::to_string(_teachers.size()) << std::endl;
		
		for(int i = 0; i < _teachers.size(); i++) {
			_teachers[i]->Teacher::save(ofs);
		}
...
void Mainwin::on_open_click() {
	...
	if(button_press == 1){
		try{
			...
			for(int i = 0; i < size; i++){
				_teachers.push_back(new Teacher{ifs});
			}
