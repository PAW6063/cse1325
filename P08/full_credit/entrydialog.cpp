...
SectionDialog::SectionDialog(const Glib::ustring& title, Gtk::Window& parent, bool modal, bool use_header_bar)
	: l_course{"Course"}, l_semester{"Semester"}, l_year{"Year"}
{//Finish here down maybe just add another constructer to existing class and then rename the class
	for(int i = 0; i < 7; i++) {
		c_subject.append(to_string(subject_list(i)));
	}
